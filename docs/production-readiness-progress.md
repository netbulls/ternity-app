...
- **~1042 unit/integration tests, all green** (shared **140**, api **902**), 61 test files. `tsc --noEmit` passes; build clean.
...
- **Portable guards in `tooling/`** (copy-paste reusable across Ternity-stack repos) — ESLint preset (`tooling/eslint`, bans casting `request.body`, in-editor) + dependency-free source scanners (`tooling/guards/source-guards.js`: unvalidated body, body casts, `sql ANY(${…})`, plugins missing `fp()`, **POST-create-without-201**) + adoption README. Dogfooded here: root `eslint.config.js` spreads the preset; `body-validation.guard.test.ts` + `architecture.guard.test.ts` call the scanners. The 201 guard pins the convention E2E surfaced — a creator POST without `reply.code(201)` (and without a `// @status-code 200` opt-out documenting why) fails the build.
- **Seal by construction** — the body-validation guard (now backed by `tooling/`) fails if any handler reads `request.body` without `.parse`/`.safeParse` (or casts with `as`). Locks in S4. The architecture guard additionally pins the `sql ANY(${…})` bug class, the plugin-`fp()` requirement, and (new) the resource-creating-POST → 201 convention. Sanity-checked: removing `reply.code(201)` from `timer.ts` made the guard report `apps/api/src/routes/timer.ts POST /api/timer/start`; restoring it returns the suite to green.
...
- **Audit re-tightened (2026-06-02)** — CI flagged one new critical + two new high:
  - **Critical**: vitest <4.1.0 (UI server arbitrary file read/exec, GHSA-5xrq-8626-4rwp). Bumped both `apps/api` and `packages/shared` from `^3.0.0` → `^4.1.0`. Vitest-4 surfaced two latent issues fixed at the same time: (a) `vi.mock(...)` inside `beforeEach` is hoisted but Vitest 4 now warns and will error — moved to top-level in `jira-client`, `toggl/client`, `timetastic/client`, `changelog-parser` tests; (b) `vi.restoreAllMocks` no longer clears call-count history on `vi.fn()` instances declared inside `vi.mock(...)` factories — added explicit `vi.clearAllMocks()` in the jira refresh-token suite where two consecutive tests both mock `db.update`.
  - **High**: axios <1.16.0 (NO_PROXY IPv4-mapped IPv6 bypass + prototype-pollution MITM via `config.proxy`, both transitive through `twilio`). Override `axios@>=1.0.0 <1.16.0` → `1.16.0` in root `pnpm.overrides`.
  - 902 api + 140 shared unit tests green; 5/5 Playwright happy paths green; `pnpm audit --audit-level=high` clean (9 moderate remain, dev/build tooling).
