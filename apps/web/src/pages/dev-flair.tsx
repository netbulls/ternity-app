...
import { Play, Square, Check, X, ChevronDown } from 'lucide-react';
      ...
      {Object.values(values).map((entry) => {
        ...
        return (
          ...
          >
            ...
            <div className="relative z-10 flex-1 min-w-0">
              <AnimatePresence mode="wait">
                {isEditingDesc ? (
                  ...
                  >
                    <motion.input
                      className="flex-1 rounded-md bg-muted/40 px-2.5 py-1 text-[13px] text-foreground outline-none"
                      style={{
                  ...
                  <motion.span
                    key="display"
                    initial={false}
                    className={`cursor-pointer truncate text-[13px] ${noDesc ? 'italic text-muted-foreground' : 'text-foreground'} hover:text-primary`}
              ...
              <div className="mt-1 flex items-center gap-1.5">
                {isEditingProject ? (
                  ...
                  >
                    <motion.div
                      className="flex items-center gap-1.5 rounded-full bg-muted/40 px-2.5 py-0.5"
            ...
            {/* Time range */}
            <div className="relative z-10 flex items-center gap-1 text-right shrink-0">
              {isEditingTime ? (
                <motion.div
                ...
                >
                  <motion.input
                    className="w-[52px] rounded-md bg-muted/40 px-1.5 py-0.5 text-center text-[11px] tabular-nums text-foreground outline-none"
          ...
          >
            ...
            <div className="relative z-10 flex-1 min-w-0">
              {isEditingDesc ? (
                ...
                >
                  <input
                    className="w-full rounded-none border-0 border-b-0 bg-transparent px-0 py-1 text-[13px] text-foreground outline-none"
                    value={entry.description}
                    onChange={(e) =>
                      setValues((v) => ({
                        ...v,
                  ...
                  <div className="absolute bottom-0 left-0 right-0 flex justify-end gap-1 -mb-6">
                    <motion.button
                      initial={{ opacity: 0, y: -4 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.85 }}
                      onClick={handleCancel}
                      className="flex h-5 w-5 items-center justify-center rounded-full text-muted-foreground hover:text-foreground text-[10px]"
                    >
                ...
                <span
                  className={`cursor-pointer truncate text-[13px] ${noDesc ? 'italic text-muted-foreground' : 'text-foreground'} transition-colors hover:text-primary`}
                  onClick={() => setEditing({ entryId: entry.id, field: 'description' })}
                >
                  {entry.description || 'No description'}
                </span>
              ...
              <div className="mt-1 flex items-center gap-1.5">
                {isEditingProject ? (
                  <div className="relative">
                    <div
                      className="flex items-center gap-1.5 rounded-full bg-transparent px-0 py-0.5"
                      style={{ fontSize: scaled(11) }}
                    >
                      <span
                        className="h-2 w-2 rounded-full"
            ...
            <div className="relative z-10 flex items-center gap-1 text-right shrink-0">
              {isEditingTime ? (
                <div className="relative flex items-center gap-1">
                  <div className="relative">
                    <input
                      className="w-[52px] rounded-none border-0 bg-transparent px-1.5 py-0.5 text-center text-[11px] tabular-nums text-foreground outline-none"
                      value={entry.startTime}
                      onChange={(e) =>
                        setValues((v) => ({
                          ...v,
