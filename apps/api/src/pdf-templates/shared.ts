...
type DayGroup = ReportData['userDetails'][number]['dayGroups'][number];
type Entry = DayGroup['entries'][number];

/**
 * Collect all visual rows for a user's time entries.
 * Iterates dayGroups → entries, calling the provided render callbacks.
 * Returns a flat array of HTML strings (one per visual row).
 */
export function collectUserRows(
  user: ReportData['userDetails'][number],
  renderers: {
    renderDayHeader: (dg: DayGroup) => string;
    renderEntry: (entry: Entry, dg: DayGroup) => string;
  },
): string[] {
  const rows: string[] = [];
  for (const dg of user.dayGroups) {
    rows.push(renderers.renderDayHeader(dg));
    for (const entry of dg.entries) {
      rows.push(renderers.renderEntry(entry, dg));
    }
  }
  return rows;
}
...
/**
 * Chunk result from paginateRows.
 */
export interface PageChunk {
  /** Joined HTML of the rows in this chunk */
  rows: string;
  /** Whether this is the first chunk for this user */
  isFirst: boolean;
  /** Whether this is the last chunk for this user */
  isLast: boolean;
}

/**
 * Split a flat array of row HTML strings into page-sized chunks.
 * First chunk uses `page1Capacity`, subsequent chunks use `contCapacity`.
 * Yields PageChunk objects for each page.
 */
export function* paginateRows(
  ...
  while (rowIdx < allRows.length) {
    const capacity = isFirst ? page1Capacity : contCapacity;
    const chunkEnd = Math.min(rowIdx + capacity, allRows.length);
    const rows = allRows.slice(rowIdx, chunkEnd).join('\n');
    const isLast = chunkEnd >= allRows.length;

    yield { rows, isFirst, isLast };
